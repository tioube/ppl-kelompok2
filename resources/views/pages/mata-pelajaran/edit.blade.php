                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="kategori" id="kategori"
                            ...
                            @foreach(['Wajib', 'Peminatan', 'Lintas Minat'] as $kat)
                                <option value="{{ $kat }}" {{ old('kategori', $mataPelajaran->kategori) == $kat ? 'selected' : '' }}>
                                    {{ $kat }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="jam_pelajaran" id="jam_pelajaran"
                            value="{{ old('jam_pelajaran', $mataPelajaran->jam_pelajaran) }}"
                            min="1" max="20"
                    ...
                    <div>
                        ...
                        <input type="number" name="preferred_block" id="preferred_block"
                            value="{{ old('preferred_block', $mataPelajaran->preferred_block) }}"
                            min="1" max="3"
                    ...
                    <div>
                        ...
                        <input type="number" name="max_per_day" id="max_per_day"
                            value="{{ old('max_per_day', $mataPelajaran->max_per_day) }}"
                            min="1" max="5"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                            class="block w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-800 dark:text-white @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $mataPelajaran->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
