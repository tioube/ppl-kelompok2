...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Time Slot" />
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <select name="day" id="day"
                            ...
                            @foreach($days as $day)
                                <option value="{{ $day }}" {{ old('day', $timeSlot->day) === $day ? 'selected' : '' }}>{{ ucfirst($day) }}</option>
                            @endforeach
                        </select>
                        @error('day')
                    ...
                    <div>
                        ...
                        <input type="number" name="slot_index" id="slot_index" min="1" value="{{ old('slot_index', $timeSlot->slot_index) }}"
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type }}" {{ old('type', $timeSlot->type) === $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="time" name="start_time" id="start_time" value="{{ old('start_time', substr($timeSlot->start_time, 0, 5)) }}"
                    ...
                    <div>
                        ...
                        <input type="time" name="end_time" id="end_time" value="{{ old('end_time', substr($timeSlot->end_time, 0, 5)) }}"
