...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Penugasan Guru" />

    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            <form action="{{ route('guru-mapel-kelas.update', $assignment) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <select name="tahun_ajaran_id" id="tahun_ajaran_id"
                            ...
                            @foreach($tahunAjaranList as $ta)
                                <option value="{{ $ta->id }}" {{ old('tahun_ajaran_id', $assignment->tahun_ajaran_id) == $ta->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="guru_id" id="guru_id"
                            ...
                            @foreach($gurus as $guru)
                                <option value="{{ $guru->id }}" {{ old('guru_id', $assignment->guru_id) == $guru->id ? 'selected' : '' }}>
                                    {{ $guru->name }}
                    ...
                    <div>
                        ...
                        <select name="mata_pelajaran_id" id="mata_pelajaran_id"
                            ...
                            @foreach($mataPelajarans as $mapel)
                                <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $assignment->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="kelas_id" id="kelas_id"
                            ...
                            @foreach($kelas as $k)
                                <option value="{{ $k->id }}" {{ old('kelas_id', $assignment->kelas_id) == $k->id ? 'selected' : '' }}>
                                    {{ $k->nama }}
                                </option>
                            @endforeach
                        </select>
