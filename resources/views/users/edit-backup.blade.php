                ...
                <div class="mt-6 border-t border-gray-200 pt-6 dark:border-gray-800">
                    ...
                    <div>
                        ...
                        <div class="mt-8 border-t border-gray-200 dark:border-gray-700 pt-8">
                            <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-300">Revoked Permissions (Block Access)</label>
                            <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
                                These permissions will be explicitly denied, even if granted through roles. Use this to remove specific permissions from users while keeping their roles intact.
                            </p>
                            ...
                            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                                @foreach($permissionCategories as $category => $slugs)
                                    @php $categoryPermissions = $permissions->whereIn('slug', $slugs); @endphp
                                    @if($categoryPermissions->count() > 0)
                                        <div class="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/10">
                                            <div class="mb-3 flex items-center justify-between">
                                                <h4 class="text-sm font-medium text-red-900 dark:text-red-300">{{ $category }}</h4>
                                                <div class="flex gap-2">
                                                    <button type="button" onclick="selectAllRevoked('{{ Str::slug($category) }}')" class="text-xs text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
                                                        Revoke All
                                                    </button>
                                                    <button type="button" onclick="selectNoneRevoked('{{ Str::slug($category) }}')" class="text-xs text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
                                                        Restore All
                                                    </button>
                                                </div>
                                            </div>
                                            <div class="space-y-2" data-category-revoked="{{ Str::slug($category) }}">
                                                @foreach($categoryPermissions as $permission)
                                                    <div class="flex items-start">
                                                        <div class="flex items-center h-5">
                                                            <input
                                                                ...
                                                                class="h-4 w-4 rounded border-red-300 text-red-600 focus:ring-red-500 dark:border-red-600 dark:bg-red-900/20"
                                                            >
                                                        </div>
                                                        <div class="ml-3 flex-1">
                                                            <label for="revoked_permission_{{ $permission->id }}" class="text-sm font-medium text-red-700 dark:text-red-300 cursor-pointer">
                                                                {{ $permission->name }}
                                                            </label>
                                                            @if($permission->description)
                                                                <p class="text-xs text-red-600 dark:text-red-400">{{ $permission->description }}</p>
                                                            @endif
                                                        </div>
                                                    </div>
                                                @endforeach
                            ...
                            <div class="mt-4 rounded-lg border-l-4 border-red-400 bg-red-50 p-4 dark:bg-red-900/20 dark:border-red-600">
                                <div class="flex">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="ml-3">
                                        <h4 class="text-sm font-medium text-red-800 dark:text-red-300">Warning: Revoked Permissions</h4>
                                        <p class="mt-1 text-sm text-red-700 dark:text-red-400">
                                            Revoked permissions will deny access even if the user has the permission through their role. This overrides all other permission grants and should be used carefully.
                                        </p>
                                    </div>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Functions for revoked permissions
    window.selectAllRevoked = function(categorySlug) {
        const container = document.querySelector(`[data-category-revoked="${categorySlug}"]`);
        if (container) {
            const checkboxes = container.querySelectorAll('input[name="revoked_permissions[]"]');
            checkboxes.forEach(cb => cb.checked = true);
        }
    };

    window.selectNoneRevoked = function(categorySlug) {
        const container = document.querySelector(`[data-category-revoked="${categorySlug}"]`);
        if (container) {
            const checkboxes = container.querySelectorAll('input[name="revoked_permissions[]"]');
            checkboxes.forEach(cb => cb.checked = false);
    ...
    const revokedCheckboxes = document.querySelectorAll('input[name="revoked_permissions[]"]');
    revokedCheckboxes.forEach(cb => {
        cb.addEventListener('change', function() {
            const permissionSlug = this.getAttribute('data-slug');
            const directPermissionCheckbox = document.querySelector(`input[name="permissions[]"][data-slug="${permissionSlug}"]`);

            if (this.checked && directPermissionCheckbox) {
                // If we're revoking a permission, uncheck the direct permission checkbox
                directPermissionCheckbox.checked = false;
                updatePermissionSummary();
            }
        });
    });

    // Add event listeners to direct permission checkboxes to handle conflicts with revoked
    permissionCheckboxes.forEach(cb => {
        cb.addEventListener('change', function() {
            const permissionSlug = this.getAttribute('data-slug');
            const revokedPermissionCheckbox = document.querySelector(`input[name="revoked_permissions[]"][data-slug="${permissionSlug}"]`);

            if (this.checked && revokedPermissionCheckbox && revokedPermissionCheckbox.checked) {
                // If we're giving direct permission, uncheck the revoked permission checkbox
                revokedPermissionCheckbox.checked = false;
            }
        });
    });
});
